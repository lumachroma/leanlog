...
function AppHeader({ activePage, onPageChange }) {
  ...
  return (
    <>
      ...
      >
        <div className="grid w-full min-w-0 grid-cols-5 gap-0.5 rounded-[1.3rem] border border-border/80 bg-background/95 p-1 shadow-sm sm:flex sm:gap-1 sm:rounded-[1.5rem] sm:p-1.5 sm:items-center">
          {navItems.map(({ label, page, active, Icon }) => (
            <Button
              ...
              aria-pressed={active}
              className="h-auto min-h-12 w-full min-w-0 shrink rounded-[0.9rem] px-0.5 py-1.5 sm:flex-1 sm:min-h-10 sm:rounded-full sm:px-4 sm:py-0"
            >
